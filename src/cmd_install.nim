...
proc install_symlink*(p_options:seq[string]) = 
  ...
  var srcPath = path
  var destPath = absolutePath(fmt"{symlinkPath}/{full_name}",root=getEnv("PathTool"))
  
  if not symlinkExists(absolutePath(fmt"etc/alternatives/{fullname}", root=getEnv("PathTool"))):
    ...
    var truePath = absolutePath(fmt"{configPath}/{name}.txt",root=getEnv("PathTool"))
    ...
    var pp = absolutepath(fmt"dpkg/alternatives/{name}.txt", root=getEnv("PathTool"))
    var old_lines:seq[string] = to_seq(lines(pp))
    var new_lines:seq[string] = old_lines
    #echo "--初始--"
    #echo "旧行: " , old_lines
    #echo "新行: " , new_lines
    var truePath = absolutePath(fmt"dpkg/alternatives/{name}.txt",getEnv("PathTool"))
